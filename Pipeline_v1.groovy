def call(Map pipelineParams) {
    ...
    pipeline {
        ...
        stage('Deploy to Dev') {
           steps {
            timestamps {
            checkpoint 'Deploy into DEV'
            input message: "Do you want to deploy into DEV", ok: 'Deploy', submitter: 'STIP-ABE-ADMIN', parameters: [
 [$class: 'TextParameterDefinition', defaultValue: 'dev', description: 'Environment', name: 'env']] 
           node('master') {
               
             script{
                    try{
                        sh '$JENKINS_SCRIPTS/queuedeploy_bbk_blw.sh queue $JOB_NAME ${BUILD_NUM}/jboss BLW dev1/jboss'
                        variables = [ job: currentBuild.rawBuild.getFullDisplayName(),
                                      job_url: "${env.BUILD_URL}" ]
                        template = libraryResource('template.html')
                        report = helper.renderTemplate(template, variables)
